...
namespace Mvc7_ViewComponents.Controllers
{
    public class InvokeController : Controller
    {
        ...
        //在View叫用View Component
        public IActionResult ViewInvokeVC()
        {
            return View(cards);
        }

        //View呼叫_VcPartial部分檢視,然後_VcPartial再呼叫View Components
        public IActionResult PvInvokeVC()
        {
            return View(cards);
        }

        //在Controller Action直接叫用View Component元件
        //不過若是直接呼叫, 變成其css或js設定必須寫在檢視元件中
        public IActionResult ActionInvokeVC()
        {
            return ViewComponent("ProductList", 10);
        }

        public IActionResult PassParameters()
        {
            return View();
        }
